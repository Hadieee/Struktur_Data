...
int fibonacciSearch(struct Node *node, string x, int n);
void tampilan_sorting(Node **Head, int array[], int pilihan);
void tampilan_urutan_sorting(Node **Head, int array[]);
void tampilan_pilihan_searching(Node **Head);
void tampilan_searching(Node **Head);
...
awal:
    ...
    case '9':
        {
            system("cls");
            tampilan_pilihan_searching(&Head);
            goto awal;
        }
    case '0':
    {
...
int fibonacciSearch(struct Node *node, string x, int n)
{
    ...
    if (F1 && head2->data.nama == x) return offset + 1;
    return -1;
}
...
searching:
    ...
    if (ind >= 0) cout << "Data ditemukan pada index : " << ind;
    else cout << Nama << " tidak ada dalam Linked List";
